...
3. **The upstream login URL** is built by the connector. For the OIDC connector, `LoginURL` (`connector/oidc/oidc.go:442`) calls `oauth2Config.AuthCodeURL(state, ...)`, putting Dex's `AuthRequest` ID into `state`, and adds PKCE parameters (Proof Key for Code Exchange, RFC 7636, which binds the authorization code to the client that started the flow).
