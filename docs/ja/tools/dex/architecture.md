...
3. **上流のログイン URL** はコネクタが組み立てる。OIDC コネクタでは `LoginURL`（`connector/oidc/oidc.go:442`）が `oauth2Config.AuthCodeURL(state, ...)` を呼び、Dex の `AuthRequest` ID を `state` に入れ、PKCE パラメータを付ける。
