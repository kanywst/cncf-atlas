...
Authelia の採用は、主に GitHub 上の存在感とセルフホスト界隈での定番性から見える。名前の挙がるエンタープライズのケーススタディからではない。2026-06-21 時点でリポジトリのスター数は約 28,100、フォーク約 1,420。README は 100 名超のコントリビュータを挙げる。

ADOPTERS ファイルは無く、本番で Authelia を運用する特定組織を名指しする公開ケーススタディも無い。2025〜2026 年の独立した比較記事はいずれも、homelab・セルフホスト界隈で目的特化の SSO / 2FA の定番として一貫して位置づけ、軽量ユーザストアの LLDAP と組み合わせる例が多い。これが採用の正直な姿: セルフホストでは広く使われ、エンタープライズの参照事例としては文書化されていない。
