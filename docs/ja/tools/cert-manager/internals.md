...
| `internal/cainjector/` | CA バンドル注入のリコンサイラ |
...
- `CertificateRequestSpec` (`pkg/apis/certmanager/v1/types_certificaterequest.go:111`)。CSR PEM を `Request` に持ち、`IssuerRef`・`Duration`・`IsCA` を伴う。発行の issuer 非依存な中間表現である。
...
ACME の失敗は原因で仕分けられる。Order の create 失敗はネットワーク起因とみなしてバックオフ・リトライする (`acme.go:160` 以降)、一方デコードできない CSR や SAN にない CommonName は `reporter.Failed` を呼んでリトライせず返す (`acme.go:122`-`:142`)。この分離が一時的障害を恒久的失敗にせず、ユーザのミスを無限ループにしない。
